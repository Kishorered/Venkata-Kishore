 ...
 *************************************************************************************************/
#define APP_STACK_SIZE                  2048        //256
#define BLE_APP_CB_QUEUE_SIZE           16
#define APP_ISR_QUEUE_SIZE              100    //2
...
#define UART_BUFF_DEPTH                 100    //2

// Advertising device name
#define DEVICE_NAME                     'E', 'F', 'L', '_', 'B', 'L', 'E', 'I', 'O', 'T'
        ...
        case BLESERVICE_TRSPS_UDATRW01_WRITE_WITHOUT_RSP_EVENT:
        {
            ...
            if (p_data != NULL)
            {
                ...
                *(p_data + p_param->length) = '\0';
                ...
                data_num=p_param->handle_num;
                memcpy(ble_data,p_data,p_param->length);
                //ei_ble_rx_data(ble_data);
                kishore_dec3(ble_data); 
                memcpy(readData,ble_data,p_param->length); 
                //
                vPortFree(p_data);
            }
        }
    ...
    case BLE_ADV_EVT_SET_ENABLE:
    {
        ...
        if (p_adv_enable->status == BLE_HCI_ERR_CODE_SUCCESS)
        {
            if (p_adv_enable->adv_enabled == true)
            {
                ...
                info_color(LOG_GREEN, "[ Running ver 0002 : [ %s ] [ %s ] ] BLE Advertising... with the name --- EFL_BLEIOT ---\n", __DATE__, __TIME__);
            }
            else
            {
                if (g_advertising_host_id != BLE_HOSTID_RESERVED)
...
// static void trsp_data_send_from_isr(uint8_t *p_data, uint8_t length)
void trsp_data_send_from_isr(uint8_t *p_data, uint8_t length)
{
...
bool uart_data_handler(char ch)
{
    ...
    rx_buffer[index++] = ch;
     info_color(LOG_DEFAULT, "-----------UART HANDLER-------------\n");
    //  memcpy(rx_buffer,"Testing_ble_rx_msg\0",sizeof(Testing_ble_rx_msg));
    //  index=15;
...
void app_init(void)
{
    ...
    info_color(LOG_DEFAULT, " ************ EFL BLE IOT ************* \n");
    info_color(LOG_DEFAULT, "------------------------------------------\n");
