...
volatile unsigned long cal_crc2=0xFFFFFFFF;
#ifdef WITH_CRC32
/* http://csbruce.com/~csbruce/software/crc32.c */
	/* This static table adds 1K */

	// static const char *TAG = "YMODEM_TAG";
...
static unsigned long crc32_2(const unsigned char* buf, unsigned long count)
{
	//unsigned long crc = 0xFFFFFFFF;
	unsigned long i;
	...
	for (i=0; i<count; i++) {
		cal_crc2 = (cal_crc2 >> 8) ^ crc_table[(cal_crc2 ^ buf[i]) & 0xFF];
	}
	return(cal_crc2 ^ 0xFFFFFFFF);
}

unsigned long calculate_file_crc(unsigned long * file_size)
{
	printf("****************crc_calculated************");
	cal_crc2 = 0xFFFFFFFF;
	unsigned long i;
	unsigned char buf[500];
	int size=1;
	// unsigned long file_byte_size=0;
	char byte;
//////
...
//////
	...
	FILE *fp = fopen("/spiffs/CNTv3.bin", "rb");
	if(fp ==NULL)
	{
	...
	while(size)
	{
	    size = fread(buf, sizeof(char), 500, fp);
		...
		// ESP_LOGI(TAG,"size Read: %d\n",size); 
		// memset(&buf[0],0,500);   
	}
	fclose(fp);
	printf("crc_calculated: 0x%08lx",cal_crc2 ^ 0xFFFFFFFF);  

	return(cal_crc2 ^ 0xFFFFFFFF);
}


unsigned long calculate_file_crc_working_in_byte(void)
{
	printf("****************crc_calculated************");
	cal_crc2 = 0xFFFFFFFF;
	unsigned long i;
...
////////
	...
	else 
	{
		...
		fseek(fp, 0, SEEK_SET);
		printf("file_size: %ld",file_byte_size);
	}

	while(size)
	{
	    // size = fread(buf, 500, 1, fp);
		...
		// 	cal_crc2 = (cal_crc2 >> 8) ^ crc_table[(cal_crc2 ^ buf[i]) & 0xFF];
		// }
		cal_crc2 = (cal_crc2 >> 8) ^ crc_table[(cal_crc2 ^ byte) & 0xFF];
		//ESP_LOGI(TAG,"size Read: %d\n",size); 
		//memset(&buf[0],0,500);   
	}
	fclose(fp);
...
//		break;  //ganesan disabled
//	case STX:   //ganesan disabled
...
static void send_packet0(char* filename, unsigned long size, unsigned long crc)
{
...
///////
	...
	while (size > 0) {
 	...
 	size = fread(data, sizeof(char), PACKET_1K_SIZE, fp);
        ...
        //delay(10);
        // cal_crc2=crc32_2(data,send_size);
		// cal_crc2=calculate_file_crc();
        printf("send_size[%ld],Block Number:[%4d],cal_crc2[0x%lx]\n",send_size, blockno,cal_crc2);
...
unsigned long ymodem_send(char* filename)
{
	int crc_nak = 1;
	int i=0;
	unsigned long crc_32bit;
    int ret=0;
    char ch;
    int timeout=0xF;
	...
	unsigned long size=0;
	printf("Ymodem send:\n");
    // crc_32bit=crc32(buf, size);
	crc_32bit=calculate_file_crc(&size);    //  
    printf("crc32:0x%08lx, len:0x%08lx\n", crc_32bit , size);
...
//	_sleep(1);
	...
	do {
		...
		ch = _getchar();
		printf("ch....:[0x%x][%d]\n",ch,ch);
	} while((ch != CRC) && (ch != -1) && (--timeout) );//(ch < 0);

	if (ch == CRC) {
        
        printf("chout:[0x%x]",ch);
        timeout=0xF;
		do {
			send_packet0(filename, size,crc_32bit);
			...
			do{
			ch = _getchar();
        printf("chin1:[0x%x] ",ch);
    }while((ch != ACK) && (ch != -1) && (--timeout) );
			if (ch == ACK) {
                        printf("chin2:[0x%x]",ch);
				ch = _getchar();
                       printf("chin3:[0x%x]",ch);
				if (ch == CRC) {

					ret = send_data_packets(buf, size);
					printf("\nsent:%s\n", filename);
					return 1;
#ifdef WITH_CRC32
					printf("crc32:0x%08lx, len:0x%08lx\n", crc32(buf, size), size);
...
void _putchar(unsigned char buf)
{
    // write_port(port, &buf, 1);
    // printf("[ %x ] ",buf);
	// int ret=send_uart_Data(TAG,&buf);
	// uart_write_bytes(UART_NUM_0, &buf, 1);   //
	uart_tx(1,buf,1);
	// printf("-%d-",tx_write_byte);
}
...
unsigned char _getchar(void)
{
    ...
    //return buffer[0]; 
	char data=0;
	// uart_read_bytes(UART_NUM_0, &data, 1, 100 / portTICK_RATE_MS);   //
	uart_rx(1,data,1);
	printf("[%x]",data);
	return data; 
}
